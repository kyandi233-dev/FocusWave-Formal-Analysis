...
- 116 个实验场次；
- 232 个正式任务区块；
- 1,392 个连续任务阶段（每区块 6 段；不等于程序的 18 试次循环）；
- 2,320 个思维探针；
- 100,224 个正式试次。
...
Q1=4 的区间跨 0。RT 水平、RT 波动与 RT slope 对 Q1 的多数区间也跨 0。这些是逐指标关联结果，不能据此推断测量维度数量或潜变量结构。
...
表注：正值为慢于参与者自身典型水平。**正式输出只提供效应估计与参与者层标准误，不提供置信区间**；本表因此不列 95% CI，避免用派生区间冒充正式统计量。Go 遗漏的可用参与者（54）与场次（93）少于治理总体（61 人 / 116 场），因为只有至少出现一次 Go 遗漏的场次才贡献事件；源表的事件计数不等于各位置有限正确反应时的样本数；不据计数相同推定各位置反应时支持相同。
...
本轮补入探针清醒等级频数 39/334/1054/893，以及区块内反应时变异系数的阶段斜率 0.0078 [0.0033,0.0122]，出处为既有正式行为附录 C 与配套运行记录，队列为 61 人/116 场。清醒等级与辨别力的系数为 0.389 [0.287,0.490]，与明确遗漏为 −0.404 [−0.564,−0.244]，二者均为 2320 探针。

旧附录反应时变异系数的主观关联模型使用 1814 探针；当前单指标有限值覆盖为 2318。新版第五章不搬用旧的 1814 探针系数，也不将较新的有限值覆盖视为该模型已经重跑的证据。完整最新主观关联表仍需交接。见 `../工作表/第五章撰写与结果来源核对_20260913.md`。
