...
### 6.1 RT-CV 定义统一（2026-09-13 查因并已修正）

**当前状态：本节全部 RT-CV 结果已统一为"30 s 窗口内至少 2 次有效正确 Go 反应"的定义，与第 5.2 节及 5.6–5.7 章一致。**

修正前，本模块的行为链接模型中 RT-CV 行只有 **1,794** 个窗口（111 个场次），而正确 Go RT 中位数行为 2,298、Go omission / No-Go commission / d′ 各为 2,300。缺口来源已查明，**不是程序错误，也不是数据丢失**：

| 步 | 事实 |
|---|---|
| 1 | 行为链接模型来自 RGB 5.5 分析，该次运行于 **2026-08-30** |
| 2 | 该次运行消费的行为表 SHA-256 为 `677524c8…`，与当前 `Behavior/formal_v3/probe_primary_30s.csv`（`b15acd4b…`）不一致，而与 `formal_v3_backup_20260912_144308` 完全一致 |
| 3 | 备份表 `rt_cv_min_n = 20` → RT-CV 有限值 **1,814**；当前表 `rt_cv_min_n = 2` → **2,318** |
| 4 | 1,814 − 20（`missing_source_rows`）= **1,794** ✔；2,318 − 20 = **2,298** ✔；2,320 − 20 = **2,300** ✔ |
| 5 | 逐探针比对（唯一键 2,320 行全匹配）：当前可估而备份不可估 **504**，反向 **0**，备份表对自身阈值零违反 |

**研究方裁定**：`rt_cv_min_n = 2` 为唯一正确口径；`= 20` 属试错产物，作废。据此已用当前行为表重跑 RGB 5.5 分析（输出到独立新根，未覆盖旧产物），并据此重新物化本模块。

**重跑结果 —— 实质变化仅限 6 行 RT-CV**
...
其余文件的差异均为约 1e-14 量级的浮点末位噪声（模型重跑非逐位可复现，已单独登记）。`n_rows` 与 `session_n` 的变化**只出现在 6 行 RT-CV 上**。

**修正后的 RT-CV 结果（n = 2,298）**

| 结果变量 | B | 95% 置信区间 | 是否排除 0 |
|---|---:|---|---|
| 整体身体动作能量 | +0.001136 | [−0.002183, +0.004456] | 跨 0 |
| 姿态横向向右 | +0.000002 | [−0.000033, +0.000036] | 跨 0 |
| 姿态纵向向上 | +0.000027 | [−0.000022, +0.000075] | 跨 0 |
| 姿态径向接近方向分数 | −0.000301 | [−0.017467, +0.016866] | 跨 0 |

**结论未变**：四条动作 RT-CV 关系的区间全部跨 0，**整体身体动作强度与 RT-CV 之间仍无可分辨的稳定线性对应**。3.6 节的姿态方向敏感性结论亦不受影响（所有相关区间在修正前后均跨 0）。

**版本处置**：修正前的 `FormalScience/Movement` 完整保留为 `FormalScience/Movement_backup_20260830_rtcv_n20`，不删除；`movement_feature_handoff.csv` 在重跑前后**逐行相同**，因此本模块的冻结特征交接不受影响。

**追溯缺口（仍待补记）**：`rt_cv_min_n` 从 20 改为 2 这一变化**没有任何分析设计文档记录**。研究方已裁定 20 为试错产物，但仍应在方法层补记该决定，避免后续再次出现同名不同构念。

完整查因记录见 `运行记录与证据/09-13-3-Movement行为链接1794窗口缺口查因.md`。
...
另有 local-only 较大源表：`movement_within_between.csv`（约 1.4 MB）、`rgb_block_cycle_source.csv`（约 524 KB）、`rgb_session_coverage_source.csv` 及各图件源表。它们保留在本地，不在 GitHub 重复复制全文。
...
所有 13 张图均为 `generated`，PNG 300 dpi，同时存在 SVG，图内无完整标题，中文字体可读且没有 glyph missing 警告。
...
| `movement_task_progression_coefficients` | main | 主动作指标任务进程系数 | 可选 |
...
本地图片目录：`D:\Project\厚粲杯\11_数据\_FormalAnalysis\FormalScience\Movement\figures\{main|qualification|sensitivity|qc}\`。
...
1. `body_motion_energy_median` 的研究决策已经冻结，但 unified feature registry 尚未在本 P4 bundle 中物化。
