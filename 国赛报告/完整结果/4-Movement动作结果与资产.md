...
### 6.1 RT-CV 行的窗口数与定义边界（2026-09-13 查因结论）

本模块的行为链接模型中，RT-CV 行的窗口数为 **1,794**（111 个场次），而正确 Go RT 中位数行为 **2,298**、Go omission / No-Go commission / d′ 各为 **2,300**。该缺口已查明，**不是程序错误，也不是数据丢失**：

| 步 | 事实 |
|---|---|
| 1 | 本模块的行为链接模型来自 RGB 5.5 分析（`models/rgb_behavior_window_gee.csv`），该次运行于 **2026-08-30** |
| 2 | 该次运行消费的行为表 SHA-256 为 `677524c8…`，与**当前** `Behavior/formal_v3/probe_primary_30s.csv`（`b15acd4b…`）**不一致**，而与 `Behavior/formal_v3_backup_20260912_144308/probe_primary_30s.csv` **完全一致** |
| 3 | 备份表的 `rt_cv_min_n = 20` → RT-CV 有限值 **1,814**；当前表的 `rt_cv_min_n = 2` → **2,318** |
| 4 | 1,814 − 20（RGB 5.5 manifest 记录的 `missing_source_rows`）= **1,794** ✔；2,318 − 20 = **2,298** ✔；2,320 − 20 = **2,300** ✔ |
| 5 | 逐探针比对（唯一键 2,320 行全部匹配）：当前可估而备份不可估 **504**，反向 **0**，备份表对自身 n ≥ 20 阈值**零违反** |

**因此本节 RT-CV 与 d′ 的"没有稳定对应"结论必须带一个定义限定**：本节使用的 RT-CV 是**"30 s 窗口内至少 20 次有效正确 Go 反应"条件下的变异系数**（覆盖 1,814 个探针），而第 5.2 节与 5.6–5.7 章使用的是**"至少 2 次"条件下的变异系数**（覆盖 2,318 个探针）。

- 两者**不是同一个变量**：n ≥ 20 版本的中位数为 0.199、第 95 百分位 0.342；n ≥ 2 版本的中位数为 0.215、第 95 百分位 0.661。
- 因此**不得把本节的 RT-CV 结果与 5.2、5.6、5.7 的 RT-CV 结果并列陈述为同一变量的证据**。
- 本模块**不重跑**：重跑上游 producer 属明令禁止项。若需在统一定义下重做动作—行为链接，应作为新的显式登记任务提出。

**追溯缺口（已登记，未解决）**：`rt_cv_min_n` 从 20 改为 2 这一变化**没有任何分析设计文档记录**；两份 `run_manifest.json` 只各自记录取值，无一处记录变化时间、原因或影响面。该缺口不影响阈值本身是否合理（n ≥ 2 是计算变异系数的数学下限，n ≥ 20 是可选的稳定性要求），但应在方法层补记。
