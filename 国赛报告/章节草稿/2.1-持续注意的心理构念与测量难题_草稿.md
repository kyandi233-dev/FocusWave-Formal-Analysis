...
持续注意具有明显的时间动态。传统警觉递减研究发现，部分持续任务中的表现会随任务时间延长而下降，但这种变化受到事件率、辨别难度和任务性质等因素调节，不能简单归结为统一的资源耗竭（Parasuraman, 1979; See et al., 1995）。此外，同一参与者在连续任务过程中还会出现较快的时刻间波动，即在相对稳定和相对不稳定的任务状态之间变化（Esterman et al., 2013; Fortenbaugh et al., 2018）。与此同时，不同个体在平均表现和稳定性方面又存在较为稳定的差异（Unsworth & Miller, 2021）。因此，持续注意研究既需要描述任务过程中的状态性变化，也需要在统计上区分这些变化与参与者之间的稳定差异。
...
上述构念特征决定了持续注意不能由单一观测直接测得。行为测量能够提供客观、连续的任务执行记录，但同一种行为变化可能由注意维持、反应策略、抑制控制或反应标准变化等不同过程造成；主观报告能够直接询问个体当时的意识内容，却依赖个体对自身状态的觉察与报告。近期构念效度研究进一步表明，即使行为表现和心智游移报告都对持续注意要求敏感，它们之间的共同变化也不足以证明两类指标可以相互替代（Welhaf & Kane, 2024）。因此，持续注意测量的关键在于明确不同观测分别反映什么、它们在什么条件下一致或分离，以及多类证据能在多大程度上共同支持对动态状态的判断。
