...
持续注意并非边界完全统一的单一心理过程，其概念是在警觉维持、连续任务表现和注意动态等研究传统中逐步发展的。经典警觉研究主要关注个体能否在长时间监控过程中保持对少见、不可预测目标事件的检测准备，由此形成警觉维持（vigilance）的研究传统（Sarter et al., 2001）。随着研究扩展，持续注意还被用于描述连续表现稳定性、注意失误以及任务过程中的动态波动。不同范式涉及的具体认知成分并不完全相同，但“在时间中维持目标相关注意”构成其共同核心（Fortenbaugh et al., 2017）。
