...
除事件频率外，眨眼持续时间、眨眼间隔和累计闭眼时间也可描述眼睑活动，并在困倦研究中显示出一定信息价值（Caffier et al., 2003; Schleicher et al., 2008）。不同眨眼指标分别反映事件密度、单次闭眼持续程度和总体闭眼负荷，其在较短窗口中的稳定性取决于实际事件数量，因此具体研究需要根据任务结构和数据覆盖确定主要表示。
