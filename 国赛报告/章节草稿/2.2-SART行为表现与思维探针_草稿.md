...
这些指标不能被压缩成一个“注意好坏”分数。No-Go 误按同时受反应抑制、优势反应和速度—准确性权衡影响，SART 本身也不是过程纯净的持续注意测验。Seli、Jonker、Cheyne 和 Smilek（2013）指出，速度—准确性权衡（speed–accuracy trade-off）会改变 SART 指标的解释；Wilson 等（2016）则发现，提高 Go 刺激比例会使反应更快并增加 No-Go 误按，却没有同步增加任务无关思维。也就是说，客观表现对状态变化敏感，但单个行为指标并不能唯一反推出参与者当时的意识内容。
...
探针还可以把不同主观维度区分开。注意内容报告关注意识是否仍围绕当前任务，可进一步区分任务相关思考、任务无关思维或思维空白；困倦—清醒报告则描述主观警觉水平。两者可以共同变化，也可以出现分离，因此不能视作同一连续等级的两种问法（Stawarczyk & D’Argembeau, 2016; Corcoran et al., 2025）。这种区分使行为、注意内容与清醒程度能够分别进入解释，而不是被合并成一个模糊的“专注程度”。
