...
经典 SART 要求参与者对频繁出现的 Go 刺激持续作答，仅在少数 No-Go 刺激出现时抑制反应。高频 Go 容易形成优势反应，低频 No-Go 则要求参与者在连续执行中保持任务规则并及时停止反应。Robertson 等（1997）发现，SART 表现与其他持续注意测验及日常注意失败报告存在联系；Manly 等（1999）进一步讨论了长时间任务中持续监控自身反应的重要性。SART 因而提供了一个具有明确时间结构的持续任务情境，可以同时观察反应速度、反应稳定性以及不同类型的行为错误。

SART 并不是过程纯净的“持续注意测验”。No-Go 试次同时包含反应抑制要求，误按还会受到优势反应强度、反应速度和策略影响。研究表明，No-Go 误按可随速度—准确性权衡（speed–accuracy trade-off）改变，调整 Go 刺激比例也能够同时改变反应速度和误按率，却不必同步改变任务无关思维（Seli et al., 2013a, 2013b; Wilson et al., 2016）。因此，SART 更适合被理解为一个对持续注意变化敏感、同时包含反应抑制和策略成分的持续任务范式。正确 Go 反应时、反应时变异、Go 遗漏、No-Go 误按以及辨别力 d′ 分别描述任务表现的不同侧面，不能由其中某一个指标直接推断单一心理状态。
...
思维探针还可以为连续行为和生理信号提供时间锚点。既有研究常围绕探针向前截取近期时间窗，将反应时、错误以及连续生理信号与随后报告的主观状态对应；不同研究采用的窗口长度并不一致，例如 10 s、20 s 或更长窗口均有使用（Andrillon et al., 2021; Corcoran et al., 2025）。这说明探针前窗口是一种常见的对齐方式，但不存在由文献直接规定的唯一标准长度。具体窗口需要在状态的时间局部性与可用于稳定估计的行为、事件和连续信号数量之间权衡。
