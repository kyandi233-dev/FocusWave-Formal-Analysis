...
连续、低干扰的非接触测量为填补探针之间的时间空白提供了新的可能。瞳孔和眨眼反映眼部与自主神经活动，身体动作提供可见外显状态线索，心率和呼吸相关活动则描述更广义的自主生理变化。这些信号可以在不频繁中断任务的情况下连续获得，但它们同样不是注意的专属标记：光照、姿态、困倦、心理负荷、个体习惯和测量条件都可能改变相同信号。多模态测评的价值由此不在于传感器数量本身，而在于不同信息能否在已有任务行为之外提供稳定、可解释的新增信息。
