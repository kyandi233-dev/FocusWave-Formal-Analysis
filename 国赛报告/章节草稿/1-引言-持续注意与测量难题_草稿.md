...
![图1 持续注意的动态性与测量难题](../assets/fig1_sustained_attention_dynamic_measurement.png)

**图 1 持续注意的动态性与测量难题。** 上图（A）示意同一过程可分解为**较慢变化**（整场尺度的漂移）与**短时波动**（局部起伏）；下图（B）比较**任务行为、思维探针、连续传感信号**三类观测的时间结构，浅色区间示意每个思维探针之前参与对应的窗口。**轨迹与采样结构均为概念示意**，不代表本研究已恢复连续注意状态或获得相应预测结果。
