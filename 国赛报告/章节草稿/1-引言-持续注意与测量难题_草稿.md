...
连续眼部活动提供了进一步观察这种动态的可能。瞳孔变化同时受到视觉生理条件、自主神经活动、唤醒、心理努力和认知控制影响，并与蓝斑—去甲肾上腺素系统（locus coeruleus–norepinephrine system，LC-NE）相关的神经调节状态存在联系（Mathôt, 2018; Joshi & Gold, 2020）。持续注意研究表明，瞳孔的总体水平、波动及时间变化轨迹都可能随任务投入和注意状态改变，而不同任务中的效应方向会受到任务情境、警觉程度和时间进程调节（van den Brink et al., 2016; Pelagatti et al., 2025）。眨眼则以离散事件形式反映眼睑行为，其事件数量、发生频率和持续特征同时受到视觉加工需求、任务组织、疲劳和困倦等因素影响（Maffei & Angrilli, 2018）。瞳孔与眨眼由此分别提供近连续的眼内生理动态和离散的眼睑行为信息，使探针时点之前的注意状态能够获得比单一行为结果更丰富的时间描述。
...
基于上述问题，FocusWave 以 SART 与思维探针构成任务内的行为和主观参照，在统一任务时间轴上同步记录近红外（near-infrared，NIR）眼部视频、RGB（可见光）视频和毫米波（millimeter-wave，mmWave）信号。眼部信息由瞳孔与眨眼动态构成，RGB 视频同时提供可见身体活动，毫米波用于感知胸部微动并估计心率和呼吸率等心肺信息。研究首先分别考察各类观测与任务进程、行为表现、Q1 注意内容和 Q2 困倦—清醒程度之间的关系，再在共同探针窗口和参与者独立验证条件下比较各类信息的单独预测能力及其在已有行为信息基础上的增量价值。FocusWave 由此旨在构建并验证一套能够在较低干扰条件下追踪持续注意动态、同时保留不同信息来源解释路径的无感多模态注意力测评系统。
