...
4.5 的技术颗粒度现按“正文保留科学解释、附录承载实现参数”组织。正文保留 YOLO26n、RITnet、Motion Energy、EAR、VMD、主要生理频段、行为阈值、探针窗口和留一参与者交叉验证等会影响测量或统计解释的内容；网络训练规模、ROI 与模型输入尺寸、滤波器阶数、VMD 参数和内部候选窗口等复现性实现细节集中到 [`../附录草稿/A-分析算法与关键实现参数.md`](../附录草稿/A-分析算法与关键实现参数.md)。毫米波实现参数在最终报告冻结前仍需按照权威生产代码再次核对。

正式研究总体样本口径按完整实验资料书写：第一阶段预实验 11 名参与者，第二阶段预实验 17 名参与者完成 20 次实验，正式实验 63 名参与者完成 119 次实验。当前已整理的分析子集不替代正式总体；结果章按每项分析实际使用的参与者数、实验记录数和有效观测数报告。

第 5 章已建立结果结构和证据边界，尚不填入未经正式运行确认的统计结果。结果顺序为：5.1 分析样本与各模态有效观测 → 5.2 行为表现与思维探针 → 5.3 近红外瞳孔 → 5.4 毫米波胸部微动 → 5.5 RGB 可见行为 → 5.6 多模态增量与互补信息。

正文继续执行《竞赛与学术报告撰写规则》及 `正式报告/2-章节融合工作要求.md`。当前固定要求包括：不写具体实验地点和内部身份管理语言；不使用 BB/BBB 等内部简称；不使用“下一节将说明”等章节导航式元话语；采用肯定式研究方法陈述；整章先检查信息递进、段落功能、跨节重复和方法—结果对应关系，再进行句子级润色。
